...
module.controller('KbnNetworkVisController', function ($scope, $sce, $timeout, Private) {
    ...
    $scope.drawColorLegend = function (canvasP, usedColors, colorDicc) {
        var canvas = canvasP.canvas
        var context = canvas.getContext("2d");
        // It happens that, in addition to Windows scaling that we might or might not run into,
        // there's a set of transforms applied--I don't know where they come from. The origin is
        // (roughly) in the center, and scaling is substantial, meaning that one unit in the context
        // plane is much smaller than a screen pixel.
        // Scaling is in transform.a and transform.d (b and c are 0); translation is in e (x) and f (y).
        // The transform was generated by composing translation and scaling, in that order. To
        // get the inverse, undo the scaling, then undo the translation, no?
        // DOMMatrix.
        var saveTransform = context.getTransform();
        // Generate the inverse transform. We need this if we want to drop our legend box in the upper-left
        // corner, for example.
        var inverseTransform;
        if (saveTransform) {
            context.resetTransform();
            context.scale(1.0 / saveTransform.a, 1.0 / saveTransform.d);
            context.translate(-saveTransform.e, -saveTransform.f);
            inverseTransform = context.getTransform();
        }
        context.setTransform(saveTransform);
        // origin is the actual top left
        var origin = {x: 0, y: 0};
        var legendTL = {x: 10, y: 20};
        if (inverseTransform) {
            origin = inverseTransform.transformPoint(origin);
            legendTL = inverseTransform.transformPoint(legendTL);
        }
        
        context.setTransform(saveTransform);

        
        var headerFontSize = Math.round(20 * inverseTransform.d);
        var headerFontName = "bold " + headerFontSize + "px Arial";
        var headerFontHeight = 0;
        var bodyFontSize = Math.round(18 * inverseTransform.d);
        var bodyFontName = "bold " + bodyFontSize + "px Arial";
        var lineSpacing = 0;
        var totalHeight = 0;
        context.font = headerFontName;
        var textMetric = context.measureText("COLORS:");
        var maxWidth = 0;
        // This is in CSS pixels, so requires no transforms.
        var headerBaseline = textMetric.actualBoundingBoxAscent;
        totalHeight += textMetric.actualBoundingBoxAscent + textMetric.actualBoundingBoxDescent + 4 * inverseTransform.d;
        headerFontHeight = totalHeight;
        if (textMetric.width > maxWidth) {
            maxWidth = textMetric.width;
        }
        context.font = bodyFontName;
        for (var key in colorDicc) {
            textMetric = context.measureText(key);
            var height = textMetric.actualBoundingBoxAscent + textMetric.actualBoundingBoxDescent;
            totalHeight += 4 * inverseTransform.d + height;
            if (lineSpacing == 0) {
                lineSpacing = 4 * inverseTransform.d + height;
            }
            if (textMetric.width > maxWidth) {
                maxWidth = textMetric.width;
            }
        ...
        context.fillRect(legendTL.x, legendTL.y, maxWidth + 4 * inverseTransform.a, totalHeight + 2 * inverseTransform.d);

        context.fillStyle = "black";
        context.font = headerFontName;
        context.textAlign = "start";
        context.fillText("COLORS:", legendTL.x + inverseTransform.d, legendTL.y + inverseTransform.d + headerBaseline);
        var p = legendTL.y + 4 * inverseTransform.d + headerBaseline + lineSpacing;
        for (var key in colorDicc) {
            context.fillStyle = colorDicc[key];
            context.font = bodyFontName;
            context.fillText(key, legendTL.x + inverseTransform.d, p);
            p += lineSpacing;
        }
    }
                ...
                if ($scope.vis.aggs.bySchemaName['first'].length >= 1 && !$scope.vis.aggs.bySchemaName['second']) {
                    ...
                    network.on("afterDrawing", function (canvasP) {
                        ...
                        if ($scope.vis.aggs.bySchemaName['colornode'] && $scope.vis.params.showColorLegend) {
                            $scope.drawColorLegend(canvasP, $scope.usedColors, $scope.colorDicc);
                        }
                    });
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    /////////////////////////////////////////////////////////////////////NODE-RELATION Type/////////////////////////////////////////////////////////////////////
